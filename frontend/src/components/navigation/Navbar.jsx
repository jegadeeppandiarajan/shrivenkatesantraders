...
const Navbar = () => {
  ...
  const getFallbackResponse = (userInput) => {
    ...
    if (input.includes('deliver') || input.includes('ship')) return "We dispatch within 24 hours across Tamil Nadu. Express delivery in Madurai area!";
    ...
    return "Welcome to Shri Venkatesan Traders! We specialize in premium industrial supplies serving businesses across Tamil Nadu. How can I help you today?";
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Logout should always work client-side even if API fails
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 transition-all duration-500 border-b border-slate-200 dark:border-dark-border ${scrolled
          ? "bg-white/95 dark:bg-dark-bg/90 shadow-xl backdrop-blur-xl"
          : "bg-white/90 dark:bg-dark-bg shadow-sm"
          }`}
      >
        {/* Accent line */}
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-brand-primary"></div>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-3 sm:px-4 py-3 lg:py-4 relative">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <NavLink
                  ...
                  to={link.to}
                  className={({ isActive }) =>
                    `relative px-5 py-2.5 text-sm font-medium tracking-wide transition-all duration-300 flex items-center gap-2 group ${isActive
                      ? "text-brand-primary bg-brand-primary/5 dark:bg-dark-card"
                      : "text-slate-700 dark:text-dark-text hover:text-brand-primary dark:hover:text-brand-primary hover:bg-brand-primary/5 dark:hover:bg-dark-hover"
                    }`
                  }
                >
                  {({ isActive }) => (
                    <>
                      <IconComponent sx={{ fontSize: 18 }} className="group-hover:scale-110 transition-all duration-300" />
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="relative p-2.5 sm:p-3 bg-slate-50 dark:bg-dark-hover hover:bg-brand-primary dark:hover:bg-brand-primary text-slate-700 dark:text-white hover:text-white transition-all duration-300 group border border-slate-200 dark:border-dark-border hover:border-brand-primary rounded-xl"
              aria-label="AI Chat"
              title="Ask our AI Assistant"
            >
              <AutoAwesomeIcon sx={{ fontSize: 20 }} className="relative z-10 group-hover:rotate-12 transition-all duration-300" />
              {aiChatOpen && (
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-brand-primary dark:bg-white"></span>
              )}
            </button>
            ...
            >
              {darkMode ? (
                <LightModeIcon sx={{ fontSize: 20 }} className="text-brand-primary group-hover:rotate-90 transition-all duration-500" />
              ) : (
                <DarkModeIcon sx={{ fontSize: 20 }} className="text-slate-700 group-hover:-rotate-12 transition-all duration-300" />
              )}
            </button>

            {/* Cart Button with bounce animation - hidden for admin */}
            {user && !isAdmin && (
              <Link
                to="/cart"
                className="relative p-2 sm:p-2.5 text-slate-700 dark:text-dark-text hover:text-brand-primary dark:hover:text-dark-bg hover:bg-slate-100 dark:hover:bg-brand-secondary rounded-2xl transition-all duration-300 group"
              >
                <ShoppingCartIcon sx={{ fontSize: 20 }} className="group-hover:scale-110 transition-transform sm:text-[22px]" />
                {totalItems > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 bg-gradient-to-r from-brand-primary to-brand-gold dark:from-red-500 dark:to-red-600 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg animate-bounce-subtle">
            ...
            {isAdmin && (
              <Link
                to="/admin"
                className="hidden md:flex items-center gap-2 px-3 sm:px-4 py-2 bg-slate-50 dark:bg-dark-card text-brand-primary dark:text-brand-secondary rounded-2xl text-sm font-bold hover:bg-brand-primary/10 dark:hover:bg-brand-secondary/20 transition-all duration-300 border border-brand-primary/20 dark:border-brand-secondary/40 hover:border-brand-primary/40 hover:scale-105 group shadow-sm dark:shadow-brand-secondary/10"
              >
            ...
            {user ? (
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-slate-50 dark:bg-dark-hover rounded-2xl border border-slate-200 dark:border-dark-border hover:border-brand-primary/50 dark:hover:border-brand-secondary hover:shadow-lg hover:shadow-brand-primary/10 dark:hover:shadow-brand-secondary/20 transition-all duration-300 group hover:scale-105"
                >
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-2xl bg-gradient-to-br from-brand-primary via-brand-gold to-brand-honey dark:from-brand-secondary dark:via-brand-honey dark:to-brand-primary text-white dark:text-dark-bg flex items-center justify-center text-xs sm:text-sm font-extrabold group-hover:scale-110 transition-transform duration-300 shadow-lg overflow-hidden">
                    {user.customAvatar ? (
                      <img src={user.customAvatar} alt={user.name} className="w-full h-full rounded-2xl object-cover" />
                    ) : user.avatar ? (
                ...
                <div className={`absolute right-0 mt-2 w-56 sm:w-64 bg-white dark:bg-dark-card rounded-2xl shadow-2xl shadow-brand-primary/10 dark:shadow-brand-secondary/20 border border-slate-200 dark:border-brand-secondary/30 overflow-hidden transition-all duration-300 origin-top-right ${userMenuOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}>
                  <div className="p-4 bg-slate-50 dark:from-dark-hover dark:via-dark-card dark:to-brand-secondary/10 border-b border-slate-100 dark:border-brand-secondary/20">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-brand-slate dark:text-dark-muted truncate">{user.email}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-2">
                    <Link
                      ...
                      className="group flex items-center gap-3 px-3 py-3 rounded-2xl text-sm font-bold text-slate-600 dark:text-dark-text hover:bg-gradient-to-r hover:from-brand-primary/10 hover:to-brand-accent/20 dark:hover:from-brand-secondary/20 dark:hover:to-brand-secondary/10 hover:text-brand-primary dark:hover:text-brand-secondary transition-all"
                    >
                  ...
                  <div className="p-2 border-t-2 border-brand-primary/10 dark:border-dark-border">
                    <button
                      onClick={handleLogout}
                      disabled={isLoggingOut}
                      className="w-full group flex items-center gap-3 px-3 py-3 rounded-2xl text-sm font-bold text-red-500 dark:text-red-400 hover:bg-gradient-to-r hover:from-red-50 hover:to-red-100/50 dark:hover:from-red-500/20 dark:hover:to-red-500/10 transition-all disabled:opacity-50"
                    >
                      {isLoggingOut ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="hidden sm:flex items-center gap-2 px-3 sm:px-4 py-2 text-slate-700 dark:text-dark-text font-bold rounded-2xl hover:text-brand-primary dark:hover:text-brand-secondary hover:bg-slate-100 dark:hover:bg-dark-hover transition-all duration-300 group"
                >
                ...
                <Link
                  to="/register"
                  className="group relative flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 bg-brand-primary hover:bg-brand-secondary text-white text-xs sm:text-sm font-bold uppercase tracking-wider transition-all duration-300 rounded-xl"
                >
            ...
            <button
              ...
              className="lg:hidden p-2 hover:bg-slate-100 dark:hover:bg-dark-hover rounded-2xl transition-all duration-300 group border border-transparent hover:border-slate-200"
              aria-label="Toggle menu"
            >
              <div className="w-5 h-5 sm:w-6 sm:h-6 flex flex-col justify-center items-center">
                <span className={`block w-4 sm:w-5 h-0.5 bg-slate-800 dark:bg-white rounded-full transition-all duration-300 ${mobileMenuOpen ? "rotate-45 translate-y-[3px]" : ""}`}></span>
                <span className={`block w-4 sm:w-5 h-0.5 bg-slate-800 dark:bg-white rounded-full my-1 transition-all duration-300 ${mobileMenuOpen ? "opacity-0 scale-0" : "opacity-100 scale-100"}`}></span>
                <span className={`block w-4 sm:w-5 h-0.5 bg-slate-800 dark:bg-white rounded-full transition-all duration-300 ${mobileMenuOpen ? "-rotate-45 -translate-y-[5px]" : ""}`}></span>
              </div>
            </button>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 h-full flex flex-col overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-6 animate-fade-in-down">
            <div className="flex items-center gap-3">
              ...
              />
              <span className="font-extrabold bg-gradient-to-r from-brand-primary to-brand-secondary dark:from-brand-secondary dark:to-brand-accent bg-clip-text text-transparent">Menu</span>
            </div>
            <div className="flex items-center gap-2">
              {/* Dark Mode Toggle in Mobile */}
              <button
                onClick={toggleDarkMode}
                className="p-2 rounded-2xl bg-gradient-to-br from-brand-light to-brand-accent/20 dark:from-dark-hover dark:to-brand-secondary/20 text-brand-primary dark:text-brand-secondary hover:shadow-lg transition-all duration-300 group border border-brand-primary/20 dark:border-brand-secondary/40"
                aria-label="Toggle dark mode"
              >
                {darkMode ? (
              ...
              <button
                onClick={() => setMobileMenuOpen(false)}
                className="p-2 hover:bg-gradient-to-r hover:from-red-50 hover:to-red-100/50 dark:hover:from-red-500/20 dark:hover:to-red-500/10 rounded-2xl transition-all group"
              >
                <svg className="w-5 h-5 text-brand-slate dark:text-dark-muted group-hover:text-red-500 dark:group-hover:text-red-400 group-hover:rotate-90 transition-all duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {user ? (
            <div className="mt-auto p-4 bg-gradient-to-br from-brand-light via-white to-brand-accent/20 dark:from-dark-hover dark:via-dark-card dark:to-brand-secondary/10 rounded-2xl border-2 border-brand-primary/20 dark:border-brand-secondary/30">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-extrabold text-slate-900 dark:text-dark-text truncate">{user.name}</p>
                  <p className="text-sm text-brand-slate dark:text-dark-muted truncate">{user.email}</p>
                </div>
              </div>
              <button
                onClick={handleLogout}
      ...
      <div className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-40 transition-all duration-300 ${aiChatOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
        <div className={`w-[calc(100vw-32px)] sm:w-80 max-w-sm rounded-2xl border-2 overflow-hidden transition-all duration-300 shadow-2xl ${darkMode ? 'bg-dark-card border-dark-border shadow-brand-secondary/20' : 'bg-white border-slate-200 shadow-brand-primary/20'}`}>
          ...
          <div className={`p-4 bg-gradient-to-r from-brand-primary via-brand-gold to-brand-honey dark:from-brand-secondary dark:via-brand-honey dark:to-brand-primary text-white dark:text-dark-bg flex items-center justify-between`}>
            <div className="flex items-center gap-2">
              <AutoAwesomeIcon sx={{ fontSize: 22 }} className="animate-pulse" />
          ...
          {/* Messages */}
          <div className={`h-64 overflow-y-auto p-4 space-y-3 ${darkMode ? 'bg-dark-bg' : 'bg-brand-cream'}`}>
            {aiMessages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-2">
                <AutoAwesomeIcon sx={{ fontSize: 40 }} className={`animate-pulse ${darkMode ? 'text-brand-gold' : 'text-brand-primary'}`} />
                <p className={`text-xs text-center font-semibold ${darkMode ? 'text-dark-text' : 'text-slate-700'}`}>
              ...
              <>
                {aiMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs px-3 py-2 rounded-lg text-xs font-semibold ${msg.sender === 'user'
          ...
          <div className={`p-3 border-t ${darkMode ? 'bg-dark-card border-dark-border' : 'bg-white border-slate-200'}`}>
            <div className="flex gap-2">
              <input
                ...
                placeholder="Ask..."
                className={`flex-1 px-3 py-2 rounded-lg border text-xs transition-all focus:outline-none focus:ring-2 focus:ring-brand-primary ${darkMode
